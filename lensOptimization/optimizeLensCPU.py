...
def generatePerfectLens(field, f):
    ...
    p = Propagator1DPadded(len(measuredField), omega, f, dx, pad_factor=3., device=torch.device('cpu')) # propagator
...
def profileLoss(field, f, weights):
    ...
    p = Propagator1DPadded(len(measuredField), omega, f, dx, pad_factor=1., device = torch.device('cpu'))
...
r = np.random.uniform(size=1000)*p/2

ropt = optGaussianProfileADAM(r, 50, 9)