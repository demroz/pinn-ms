...
def profileLoss(field, f, weights):
    ...
    loss = -torch.abs(fieldAtFocalPlane[imax])**2
    #print(loss.item())
...
def optGaussianProfileADAM(initR, F, nrads):
    ...
    et = time.time()
    print('1 batch takes', et-st)
    omega = 2*np.pi/0.633
    
    p = Propagator1DPadded(len(measuredField), omega, F, dx, pad_factor=2., device=torch.device('cuda')) # propagator
    fieldAtFocalPlane = p.prop(measuredField)
    imax = torch.argmax(torch.abs(fieldAtFocalPlane)**2)
    x = np.arange(0,len(measuredField))*dx
    x0 = x[imax]
    weights = generatePerfectLens(result, F, x0)
    for epoch in range(epochs):
        ...
        et2 = time.time()
        print('fwd sim takes ', et-st, 'fwd+inv takes', et2-st)
        print(epoch, "out of", epochs)
    plt.figure()
    plt.plot(train_loss_ar)
    plt.show()
    
    return r.detach().cpu().numpy(), result, et-st
