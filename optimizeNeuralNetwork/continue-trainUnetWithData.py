...
def nnLoss(E,F,eps, Ebar, Fbar):
    ...
    return 0.5*torch.mean(torch.abs(l1))+alpha*torch.mean(l2)
...
for epoch in range(nepochs):
    ...
    # loss
    optimizer.zero_grad()
    Ebar,Fbar = simulateFDFD(batch_eps)
    Ebar = Ebar.reshape(10,Nx*Ny).cuda()
    Fbar = Fbar.reshape(10,Nx*Ny).cuda()
    
    loss = nnLoss(nnre(batch_eps),nnim(batch_eps),batch_eps, Ebar, Fbar)
    loss.backward()
    optimizer.step()
...
#             testLoss = nnLoss(nnre(epst).cuda(), nnim(epst).cuda(),epst.cuda(), torch.tensor(Etest.reshape(10,Nx*Ny)).cuda(), torch.tensor(Ftest.reshape(10,Nx*Ny)).cuda())
...
# torch.save(nnre.state_dict(), 'trainedNets/nnre3')
# torch.save(nnim.state_dict(), 'trainedNets/nnim3')